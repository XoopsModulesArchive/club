...
class ClubPersistableObjectHandler extends XoopsObjectHandler {
    ...
    * @return void
    */
    function ClubPersistableObjectHandler(&$db, $tablename, $classname, $keyname, $idenfierName = false) {
			...
			foreach($objects as $object) {
				...
				foreach($externalKeys as $key) {
					...
					if($ret[$i][$key] != 0) {
						...
						if(!isset($cached[$externalKey['className']][$ret[$i][$key]])) {
							if($externalKey['core']) {
								$handler = xoops_gethandler($externalKey['className']);
							} else {
								$handler = xoops_getmodulehandler($externalKey['className'], 'club');
							}
							$cached[$externalKey['className']][$ret[$i][$key]] = $this->objectToArrayWithoutExternalKey($handler->$externalKey['getMethodeName']($ret[$i][$key]),$format);
						}
						$ret[$i][$externalKey['keyName']] = $cached[$externalKey['className']][$ret[$i][$key]];
					}
			    }
				$i++;
			}
